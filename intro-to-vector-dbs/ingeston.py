...
if __name__ == "__main__":
    ...
    loader = TextLoader("mediumblog1.txt")  # Here all the contents in the text file is going to be ingested in 1 document
    document = loader.load()

    # print(f"Loader: {loader} and Type of Loader: {type(loader)}")
    # # Loader: <langchain_community.document_loaders.text.TextLoader object> and 
    # # Type of Loader: <class 'langchain_community.document_loaders.text.TextLoader'>

    print(f"Document: {document} with length: {len(document)} and Type of Document: {type(document)}")  
    ...
    # it can use regex, it can use different length functions to count the number of tokens that we are splitting it for, etc
    ...
    # and we wouldn't understand anaything from it. Chunk_overlap is used to overlap between the chunks, when we don't want to lose 
    ...
    # tokens. Because another rule of llm, is "Garbage In, Garbage Out". So, if we send a lot of garbage which is uneccessary the first 
    # thing that will affect us is the cost and then the llm would throw garbage out as it got lot of uneccessary information
    ...
    # you can read them and adjust your chunk size accordingly until you think it makes sense. You can read them by debugging and 
    # evaluating it
