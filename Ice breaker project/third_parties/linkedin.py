...
Link: 
https://gist.githubusercontent.com/DCGUY009/16175ccc5daa5fa1a19b15ce9fba8044/raw/149bccd37bc0e359d60193ecc9074278c10d8161/gistfile1.txt
"""
...
# Always define a function with type of inputs defined so that it is clear for you and for others who see it, also if you want to set 
# default values like `mock`` in this function, you can set it by writing it similar to the one below
def scrape_linkedin_profile(linkedin_profile_url: str, mock: bool = False):
    ...
    data = (
        response.json()
    )  # If the user data contains a profile picture URL, it has a TTl (Time to Load) of 1 hour. After 1 hour,
    # we cannot access the profile picture through the link. So, it is best if you save it somewhere in a storage

    # There are lot of empty fields in the response and we want to remove them.
    data = {
        k: v
        for k, v in data.items()
        if v not in ([], "", "", None)
        and k not in ["people_also_viewed", "certifications"]
    }  # We are basically only considering non-empty fields and disregarding people_also_viewed and certifications section as we don't 
    # require those (objective to every use case)
